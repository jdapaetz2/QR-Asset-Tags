/**
 * Renter/staff-facing photo guidance copy (Phase 3C.4). Centralized so the renderer, review warnings, and the
 * omission dialog all read from one place and stay consistent regardless of a template's stored `help`.
 *
 ...
 * sets an expectation, the omission-confirmation flow keeps them skippable.
 */
...
/** Review-step warnings (soft, non-blocking) + the omission-dialog bodies. */
...
/**
 * Named object phrases for the template-specific overview photo slots → a direct "Add a photo showing the
 * {object} …" line. Anything not listed falls back to the general condition copy. `plural` picks its/their.
 */
...
/** Approved guidance for a photo slot, keyed by its stable slot id. */
export function photoSlotHelp(slotId: string): string {
  if (slotId === DAMAGE_PHOTOS_SLOT_ID) return PHOTO_HELP_DAMAGE;
  if (slotId === ADDITIONAL_PHOTOS_SLOT_ID) return PHOTO_HELP_ADDITIONAL;
  const named = SLOT_OBJECT[slotId];
  if (named) {
    return `Add a photo showing the ${named.object} so the rental team can verify ${
      named.plural ? "their" : "its"
    } condition.`;
  }
  return PHOTO_HELP_GENERAL;
}
