...
- delivery checks that cannot be run safely or were not reported back.
...
| 3 | Urgent routing | **CONDITIONAL GO** | Five live route decisions were correct: main only, urgent only, general-off routine skip, one send for the same address, two routes for different addresses. The database and the settings form both refused an urgent switch without an address. The main-route line of the two-address case was not viewed on its own. Partial-failure isolation is unit-tested only. |
...
| 5 | Daily summary reliability | **GO** | First real windows 2026-09-12 (quiet), 2026-09-13 (`sent`, 7 items), 2026-09-14 (quiet); one ledger row per organization per window; cron auth 3/3. DST, catch-up, failure and ordering are proven on staging. The delivered summary's content was not reported back (§7). |
...
| 7 | Delivery and client rendering | **CONDITIONAL GO** | Delivery was verified directly in Outlook and Gmail web was verified in D4. Still missing: live replay, live provider failure, cold-mailbox placement, and the Gmail review of the D5 set. A forwarded Gmail copy lost its inline image. |
| 8 | Limited-pilot readiness (notifications) | **CONDITIONAL GO** | Conditions: D5.1; the clean-return decision; the unreported operator checks (§14). Standing: `after()` is best-effort, not a queue, and Supabase has no backups (Phase C §13). |
...
- **Organizations:** three, all active.
  ...
  - QA organization: `off`, no address, tag updates off — restored to its pre-D5 values (verified 2026-09-14 16:48 UTC).
...
| **Routing (7)** | main only; urgent only (damage off); general off + routine; two addresses; same address; invalid urgent settings; partial route failure | `main`; `urgent`; `skipped_disabled`; urgent `sent` (main line not individually viewed); one `main_and_urgent`; database `23514` + settings-form error, nothing saved; **not run live** (unit-tested) |
...
**Operator-pending:** a visual check of the delivered 2026-09-13 summary email was not reported.
...
- **Gmail.** D4 Gmail web and mobile evidence (2026-09-11) stands. The Gmail review of the D5 set — including
  authentication headers, inbox or spam placement, and the support-inbox copy of `SUB-2026-0D7A41` — was **not
  reported**.
...
- **What happened.**
  ...
  - `delivered_at` was stamped by that same save; `updated_at` is 45 ms later in the same write.
  - No later save is recorded.
- **Not verified live** (operator-pending):
  - the emails those saves produced;
  - a same-status save;
  - a notes-only save.
...
| `perf:action:production --samples=6` (2026-09-14, QA organization without a recipient) — no media | POST median **547 ms** (p75 652); click → confirmation median **804 ms** (max 845) | Phase C: POST 575 ms, confirmation 834–949 ms |
...
damage reports to the QA organization; with no recipient set, it sent no email.
...
- Tag request saved to `delivered` (2026-09-14).

**Pending — not reported at closeout:**

- Gmail review of the D5 emails, with authentication headers and placement.
- Visual check of the 2026-09-13 summary email.
- The support-inbox copy of `SUB-2026-0D7A41`.
- The tag-request emails, a same-status save and a notes-only save.
- A recorded decision on cold-mailbox placement: test a never-allowlisted mailbox, or make allowlisting part of onboarding.
- The Resend tracking setting.
