...
- **Deployment & environment (A2).** Canonical **Node 22 LTS** (`.nvmrc` + `package.json` engines; local Node 24 is outside the tested baseline). `lib/env.ts` normalizes `NEXT_PUBLIC_SITE_URL` to a canonical origin and, in Vercel **production**, requires https + a non-placeholder host; `SCAN_IP_HASH_SALT` **fails closed** (≥ 32 chars) in Vercel **production/preview**. `npm run verify:production-config` runs static config checks (var names, migrations 0001–0031 present, Node version, server-only boundaries) in CI and locally, printing PASS/WARN/FAIL and never a secret value. CI runs on `main`, PRs, and `pilot-credibility`. Deploy/rollback/smoke: `docs/PRODUCTION_DEPLOYMENT_RUNBOOK.md` + `docs/PRODUCTION_SMOKE_TEST.md`. Migrations are applied manually (approval-gated `supabase db push`), never by CI. Remote 0001–0031 are operator-verified applied (`docs/MIGRATION_LEDGER.md`).
