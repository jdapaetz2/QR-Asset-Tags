...
- **Uploads.** Validate type and size; store under org-scoped paths (`org/{organization_id}/...`); never allow public listing. Apply rate limiting and/or a honeypot. **(D4.1)** Identify files by their bytes, never by extension or declared MIME (`lib/media/sniff.ts`, `lib/media/classify.ts`). Photo inputs use `usePhotoInput` with a profile from `lib/media/photo-policy.ts` and `accept={PHOTO_ACCEPT}` (`image/*,.heic,.heif,.avif` — never `image/heic`); the hook converts HEIC/AVIF/GIF/oversized photos to JPEG (PNG for logos) on the device, so stored operational images stay JPEG/PNG/WebP. The server re-verifies type, bytes, frame size (≤ 40 MP, ≤ 16,384 px) and upload age (≤ 24 h) before a row references an object. Hosted documents are never converted; HEIC/HEIF/AVIF originals are allowed (migration 0039). The HEIC decoder is the vendored libheif-js build in `public/vendor/libheif/` (`scripts/vendor-libheif.mjs`, LGPL notice in `THIRD_PARTY_NOTICES.md`); keep it out of the `/t/` bundle (`npm run check:scan-bundle`). Full matrix: `docs/STORAGE_MEDIA_LIFECYCLE.md`.
...
- **QR domain durability.** Physical tags encode `${NEXT_PUBLIC_SITE_URL}/t/{short_code}`. The domain must be a stable production/custom host before tags are produced — `localhost`/preview URLs are test-only, and changing the domain later breaks printed tags unless redirects are preserved. See `docs/QR_DOMAIN_STRATEGY.md`. **(A2) The durable-output routes (`qr.svg`, `qr-sheet.svg`, `export.csv`) fail closed** via `productionOutputBlock` (`lib/qr/output-guard.ts`) when the base URL isn't a safe https production host (`isProductionBaseUrl`/`productionBaseUrlIssue`), unless an explicit `?unsafe=1` testing acknowledgment is present; the owner Production page passes `?unsafe=1` only when it already shows the non-prod warning.
- **Deployment & environment (A2).** Canonical **Node 22 LTS** (`.nvmrc` + `package.json` engines; local Node 24 is outside the tested baseline). `lib/env.ts` normalizes `NEXT_PUBLIC_SITE_URL` to a canonical origin and, in Vercel **production**, requires https + a non-placeholder host; `SCAN_IP_HASH_SALT` **fails closed** (≥ 32 chars) in Vercel **production/preview**. `npm run verify:production-config` runs static config checks (var names, migrations 0001–0039 contiguous, Node version, server-only boundaries) in CI and locally, printing PASS/WARN/FAIL and never a secret value. CI runs on `main`, PRs, and `pilot-credibility`. Deploy/rollback/smoke: `docs/PRODUCTION_DEPLOYMENT_RUNBOOK.md` + `docs/PRODUCTION_SMOKE_TEST.md`. Migrations are applied manually (approval-gated `supabase db push`), never by CI. Remote 0001–0039 are operator-verified applied on staging and Production (`docs/MIGRATION_LEDGER.md`).
- **Notifications (Engineering Phase D).** Public submissions schedule identifiers only with `after()` (`lib/notifications/schedule.ts`). `notifySubmission` (`lib/notifications/notify.ts`) reloads the committed row and asset with the service role, refuses mismatches (`checkSavedSubmission`), projects one `NotificationBrief` (`projection.ts`), sets priority deterministically (`priority.ts`: `immediate` / `follow_up` / `routine` / `record`), resolves recipients from stored settings only (`routing.ts`: routes `main`, `urgent`, `main_and_urgent`), builds at most three metadata-stripped CID previews (`previews.ts`) and sends each recipient separately with its own idempotency key (`idempotency.ts`, `send.ts`; Preview is always `dry_run`). Return modes `instant_renter` / `daily_exceptions` / `off`; staff returns and outbound inspections never send individually. The daily return-exceptions summary is `app/api/cron/return-digest/route.ts` → `digest-worker.ts` + `digest-store.ts` + the service-role ledger `notification_digest_runs` (0036); Vercel cron at 13:00 and 14:00 UTC, only the 6 AM `America/Vancouver` hour proceeds. Tag-request emails go out only on a real status change (`lib/tags/owner-actions.ts`). Live QA tooling: `scripts/production/qa-notification-matrix.mjs`, `notification-config-report.mjs`, `tests/qa/`. Design `docs/ACTIONABLE_NOTIFICATION_DESIGN.md`; verdicts `docs/PHASE_D_NOTIFICATION_READINESS.md`.
