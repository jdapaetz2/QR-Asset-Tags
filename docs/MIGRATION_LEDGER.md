...
| 0034 | `0034_notification_routing.sql` | **Engineering Phase D3A** — `organizations.notify_urgent_reports`, `urgent_notification_email`, `return_notification_mode` (CHECK `instant_renter`/`daily_exceptions`/`off`), `notify_include_photo_previews`; urgent-route-has-address CHECK; backfill `notify_return_checklists` true → `instant_renter`, false → `off`. Legacy boolean kept (mirrored by the settings action, read by nothing; dropped later). No grant change. **Applied to staging and Production 2026-09-11** (linked-project guard, dry run, operator approval, manual Production dump beforehand) | 0012,0032 | Additive (cols + CHECKs + data backfill) | — | `lib/notifications/{settings,routing,notify}.ts`, notification settings form | `select return_notification_mode, count(*) from public.organizations group by 1;` |
| 0035 | `0035_tag_request_internal_columns.sql` | **Security fix** — column-level SELECT on `tag_requests` for `authenticated` excluding `production_notes`, `platform_viewed_at`, `platform_viewed_by_profile_id` (a new column is not customer-readable until added to the grant); `protect_tag_request_insert` BEFORE INSERT trigger coerces owner-controlled columns and the requester for non-owner callers; owner-only SECURITY DEFINER `owner_tag_request_internal(uuid)` and `mark_tag_request_viewed(uuid)` (anon execute revoked). Corrects 0011's "queries don't select them" assumption. **Applied to staging and Production 2026-09-11**, code-first (the prior code selected the internal columns directly, so `132dde7` was deployed before the migration; manual Production dump beforehand) | 0010,0011,0032 | Additive (grant change + trigger + 2 RPCs) | TRG,RPC,G/R | owner tag-request list/detail, owner console unviewed counts, `lib/tags/request-detail.ts`, customer create flow | `select has_column_privilege('authenticated','public.tag_requests','production_notes','SELECT');` → false |
| 0036 | `0036_notification_digest_runs.sql` | **Engineering Phase D3B** — private `notification_digest_runs` run ledger for the daily return-exceptions summary: one row per organization window, unique `(organization_id, digest_type, window_end)` claim, status `processing`/`sent`/`skipped_quiet`/`failed`, bounded `failure_class`; RLS enabled with no policies, anon/authenticated revoked, service_role select/insert/update. No recipient, body or item data. **Applied to staging and Production 2026-09-11** before the D3B code push (linked-project guard, dry run, operator approval, manual Production dump beforehand; staging worker check `npm run digest:staging-check` passed with a fake sender). Code `089f707` then promoted to Production (deployment `9FmryFHdh`); cron entries registered and `cron:verify-production` 3/3 | 0001 | Additive (table + index + RLS + grants) | RLS,IDX,G/R | `lib/notifications/digest-store.ts`, `app/api/cron/return-digest/route.ts` | `select has_table_privilege('authenticated','public.notification_digest_runs','SELECT');` → false |
| 0037 | `0037_submissions_direct_upload.sql` | **Storage fix** — drops the anon `submissions public insert` policy (0002) and tightens the `submissions` bucket to `file_size_limit` 10 MB and `allowed_mime_types` JPEG/PNG/WebP (was 100 MB including HEIC and video). Public photos now go browser → storage through server-issued signed upload URLs, past Vercel's 4.5 MB Function request-body limit, and are verified server-side before insert. Authenticated organization policies unchanged. **Code-first:** the direct-upload code no longer needs the anon policy, so it deploys before the migration. **Applied to staging and Production 2026-09-11/12** after `30f4d7a` was live on each (linked-project guard, dry run showing only 0037, manual Production dump beforehand; `migration list` 0001–0037 matched on both). Before and after the migration, `scripts/qa/direct-upload-verify.mjs` submitted a 5-photo 14.9 MB damage report and an 8.8 MB two-slot return through the deployed app (staging `SUB-2026-702447`/`D7B26D` then `946EBF`/`86EE06`; Production `SUB-2026-526079`/`28B2C9` then `08B536`/`557366`), every object verified in storage; after it, both projects report the bucket at 10 MB JPEG/PNG/WebP and refuse an anon direct upload. `smoke:staging` 27/0/1, `smoke:production` 13/0/1 | 0002 | Altering (policy drop + bucket update) | RLS | `lib/forms/{upload-prepare,upload-intake,media-verify,upload-client}.ts`, `lib/forms/submit.ts`, `lib/inspections/{submit,staff-return-submit,outbound-submit}.ts`, `components/public/{public-form,return-inspection-form}.tsx` | `select count(*) from pg_policies where schemaname='storage' and policyname='submissions public insert';` → 0; `select file_size_limit, allowed_mime_types from storage.buckets where id='submissions';` → 10485760, {image/jpeg,image/png,image/webp} |
| 0038 | `0038_public_assets_limits.sql` | **Storage tightening** — `public-assets` bucket to `file_size_limit` 5 MB and `allowed_mime_types` JPEG/PNG/WebP (was 25 MB including HEIC and PDF). Only cover images (≤ 5 MB) and org logos (≤ 2 MB) are stored there; hosted documents live in the private `documents` bucket since 0005. Ships with admin document/cover direct uploads (browser → storage with signed URLs minted by the user's RLS client, verified before the row references them). Existing objects unaffected; policies unchanged; compatible with the previous code. **Applied to staging 2026-09-12** after `d884da4` was live on the Preview (linked-project guard, dry run showing only 0038, `migration list` 0001–0038 matched); `scripts/qa/admin-upload-verify.mjs --target=staging` 9/9 (bucket 5 MB JPEG/PNG/WebP; 12 MB PDF hosted document saved with its object named after the document and stored at full size, then removed; 4.8 MB cover saved under the asset's cover folder at full size, original cover restored); `smoke:staging` 27/0/1. **Applied to Production 2026-09-12** after `113563f` (admin direct uploads `d884da4` + no-JavaScript fallback `af19af5`) was promoted (Production deployment `6412733923`; `smoke:production` 13/0/1): linked-project guard and `verify:production-target`, dry run showing only 0038, `migration list` 0001–0038 matched. **The manual Production dump was skipped at the operator's direction** (bucket-limit change only, no data or policy change; reversible by restoring the 0002 values). Read-only check afterwards: `public-assets` 5 MB JPEG/PNG/WebP, `documents` 50 MB unchanged, `submissions` 10 MB unchanged; the no-JavaScript damage form copy renders in order on the QA tag. `direct-upload-verify --target=production --after-0037` 12/12 (`SUB-2026-161457`, `SUB-2026-51BC5F`) confirms the shared storage refactor. Admin document/cover uploads were not exercised live on Production (no Production admin QA flow; covered on staging) | 0002 | Altering (bucket update) | — | `lib/assets/{cover,cover-storage,actions}.ts`, `lib/org/logo.ts`, `components/asset-form.tsx` | `select file_size_limit, allowed_mime_types from storage.buckets where id='public-assets';` → 5242880, {image/jpeg,image/png,image/webp} |
| 0039 | `0039_documents_heic_images.sql` | **Engineering Phase D4.1** — the private `documents` bucket also allows `image/heic`, `image/heif` and `image/avif`, so phone and camera image originals can be kept as hosted documents (never converted; offered as "Download original"). Still private, 50 MB; policies (0005, 0006) and every other bucket unchanged; existing objects unaffected; compatible with the previous code, which never sends these types. Ships with broad consumer photo inputs (HEIC/AVIF/GIF/large photos converted to JPEG on the device, identified by bytes) and server-side frame-size and upload-age checks. **Applied to staging 2026-09-12** after `3fb3364` was live on the Preview (linked-project guard, dry run showing only 0039, `migration list` 0001–0039 matched); `smoke:staging` 27/0/1; `direct-upload-verify --after-0037 --samples` 22/22 (HEIC/AVIF/GIF report `SUB-2026-BF0EC1`, HEIC/AVIF return `SUB-2026-B556D9`, stored as EXIF-free JPEG); `admin-upload-verify --after-0039 --samples` 19/19 (untyped HEIC document kept byte-for-byte as `.heic` with "Download original"; HEIC cover stored as JPEG); operator device QA on iPhone Safari, iPad Safari, Windows Chrome/Edge passed (TIFF refused, as designed). **Applied to Production 2026-09-13** after operator approval and promotion of `3fb3364` (Production deployment `6417386658`; `smoke:production` 13/0/1; `/vendor/libheif/libheif.wasm` served as `application/wasm`): linked-project guard and `verify:production-target`, dry run showing only 0039, `migration list` 0001–0039 matched; CLI relinked to staging afterwards. **The manual Production dump was skipped at the operator's direction** (bucket MIME-list change only; reversible by restoring the 0005 list). Read-only check: `documents` private 50 MB with HEIC/HEIF/AVIF, `submissions` 10 MB and `public-assets` 5 MB still JPEG/PNG/WebP. `direct-upload-verify --target=production --after-0037 --samples`: large-photo report `SUB-2026-5347B9` and return `SUB-2026-F4D3FC` passed, HEIC/AVIF return `SUB-2026-A98ECE` passed; the HEIC/AVIF/GIF report failed once with the generic upload message (form and photos kept) and passed on an immediate re-run (`--only=formats-damage`, `SUB-2026-30A18C`) — cause not captured (request diagnostics were added to the script afterwards) | 0005 | Altering (bucket update) | — | `lib/documents/{upload,storage,actions}.ts`, `components/document-form.tsx`, documents admin page, `lib/public/documents{,-server}.ts` | `select allowed_mime_types from storage.buckets where id='documents';` → includes image/heic, image/heif, image/avif |
