...
| **Stable production domain / QR base URL.** **Decided in B3: `https://mulemark.io` is the canonical product and permanent-QR host** (`getmulemark.com` and `mulemark.ca` are reserved and must never be QR destinations). The software gate is closed: the tag guard is a denylist so the domain needs no code change, path preservation and the preview/localhost blocks are locked in by tests, and canonical metadata is environment-derived. **Remaining operator actions:** add the domain + `www` to the production Vercel project, add the DNS records Vercel specifies (without disturbing Google Workspace MX/SPF/DKIM/DMARC or the Resend `notify` records), set Production `NEXT_PUBLIC_SITE_URL`, redeploy, then re-run `verify:tag-config` and the live route checks. | `docs/PRODUCTION_DOMAIN_CHECKLIST.md`, `docs/QR_DOMAIN_STRATEGY.md` | **operator: DNS + Vercel domains** |
...
| **Notification reliability + observability.** ~~Failures swallowed; no structured logs / provider IDs / retry.~~ **HARDENED in A5** (explicit outcomes, one redacted `[notifications]` log per attempt, provider message-ID capture, timeout + bounded retry) **and completed in B4:** Reply-To (`support@mulemark.io`), a deterministic Resend `Idempotency-Key` reused across retries so a timeout can no longer duplicate a customer's email, a **code-enforced** preview dry-run that holds even if a key were added to Preview, a 15 s total wall-clock budget so a slow provider cannot stall a renter's submission, and transactional templates (operational subjects, plain-text part, no images/tracking/shorteners, explicit reason line). 93 notification tests. **Still deferred (operator):** the three Production variables + redeploy, the API-key scope and tracking-off confirmations, and an actual multi-provider test through the app — **live email has still never been sent by the application.** | `lib/notifications/*`; `docs/EMAIL_*` runbooks | A5 + B4 done; **operator: Production env + live test** |
