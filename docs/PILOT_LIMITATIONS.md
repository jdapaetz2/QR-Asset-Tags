...
| ~~**Notification reliability + observability.**~~ **HARDENED in A5**, **completed in B4**, and **LIVE since the B4 operator closeout (2026-08-31).** Production now sends real, authenticated transactional email from `Mulemark <notifications@notify.mulemark.io>` with Reply-To `support@mulemark.io`; damage and support notifications verified end-to-end — one email each, provider ID captured, `mulemark.io` links, `spf`/`dkim`/`dmarc` passing at Gmail **and** Outlook. Staging sends nothing. **Not verified:** return-checklist and tag-request notifications have never sent live (different code paths), replay/idempotency is unit-tested only, the provider-failure path was not exercised live, and the Outlook Inbox result came from an **allowlisted** mailbox so cold placement is unmeasured. Two operator confirmations remain open: API-key scope and open/click tracking off. | `lib/notifications/*`; `docs/EMAIL_*` runbooks | **CONDITIONAL GO** — see readiness verdict 5 |
