...
| **Staging shares production's Supabase project + env.** Vercel scopes every variable `Production, Preview`, so a preview deployment reads and writes the production database with the production service-role key. A6.3 worked around this with a single disposable, clearly-labelled QA org plus a cleanup script, but that is a convention, not an isolation boundary. | `docs/STAGING_DEPLOYMENT_RUNBOOK.md` | **give staging its own Supabase project + preview-scoped env before a paid pilot** |
