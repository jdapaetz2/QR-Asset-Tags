...
| **Performance figures are a staging lab baseline, not production.** Small sample (5/route), one machine, one network, a `*.vercel.app` origin, and a database shared with production. Speed Insights is wired but has no meaningful field data. | `docs/PERFORMANCE_BASELINE.md` | re-baseline on the final domain with field data before quoting numbers |
...
| **Staging shares production's Supabase project + env.** Vercel scopes every variable `Production, Preview`, so a preview deployment reads and writes the production database with the production service-role key. **Phase B1A (done)** replaced the convention with enforcement: a tested target-resolution module, three verifier scripts, a Supabase CLI linked-project guard, and an explicit `MULEMARK_TARGET` precondition on every destructive script — classification is by project ref, ambiguity fails closed to production, and staging tooling refuses the production ref by name. **The coupling itself is not removed until Phase B1B**, when the operator creates the staging project and re-scopes the Preview variables. | `docs/STAGING_ENVIRONMENT_SETUP.md`; `scripts/lib/env-target.mjs` | **B1B — operator: create staging project + re-scope Preview env** |
| **Vercel builds on Node 24.x; the tested baseline is Node 22** (`.nvmrc`, `engines`, CI). Deployments are not built on the version the suites are verified against. | Vercel project settings vs `.nvmrc` | set the Vercel project to Node 22.x before the pilot deploy |
