...
**Notification email check (decided 2026-09-14).** First-contact inbox placement is handled per customer here, not
measured in general (`EMAIL_DELIVERABILITY_RUNBOOK.md` → Outlook placement):

1. Confirm the organization's notification address and switches under **Settings → Notifications** are what the
   customer wants.
2. Send the customer admin [`EMAIL_ALLOWLIST_GUIDE.md`](EMAIL_ALLOWLIST_GUIDE.md). The customer — or their mail
   administrator, for a Microsoft 365 tenant — adds `notify.mulemark.io` to the notification mailbox's safe senders.
   The operator explains what to do and does not change the customer's mailbox.
3. The customer submits a test damage report on one of their own tags and confirms the email arrived in that
   mailbox's **Inbox** (not Junk or quarantine).
4. Record the date and result in the checklist below. If it landed in junk after allowlisting, stop and follow the
   deliverability runbook before go-live.
...
- [ ] Notification email check (§3.10): allowlist guide sent; customer confirmed a test damage report reached the
      notification mailbox's Inbox — date and result recorded.
